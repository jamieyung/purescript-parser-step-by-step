module Test.ParserStepByStep.Step3 where

import Prelude

import Data.Either (Either(..))
import Data.Tuple (Tuple(..))
import ParserStepByStep.Step3 (parseFile, parseKV, runParser)
import Test.Spec (Spec, describe, it)
import Test.Spec.Assertions (shouldEqual)

spec :: Spec Unit
spec = describe "Step3" do

  describe "parseKV" do

    it "should fail if there is no first byte in the input" do
      runParser parseKV { idx: 0, arr: [] } `shouldEqual` Left "failed to get next byte"

    it "should fail if the first byte is not 1" do
      runParser parseKV { idx: 0, arr: [ 2 ] } `shouldEqual` Left "expected 1 but got 2"

    it "should fail if there is no byte for the key length" do
      runParser parseKV { idx: 0, arr: [ 1 ] } `shouldEqual` Left "failed to get next byte"

    it "should fail if the number of bytes remaining is less than what the key length byte indicates" do
      runParser parseKV { idx: 0, arr: [ 1, 10 ] } `shouldEqual` Left "Expected 10 more bytes in input"

    it "should fail if there is no byte for the value length" do
      runParser parseKV { idx: 0, arr: [ 1, 2, 65, 66 ] } `shouldEqual` Left "failed to get next byte"

    it "should fail if the number of bytes remaining is less than what the value length byte indicates" do
      runParser parseKV { idx: 0, arr: [ 1, 2, 65, 66, 10 ] } `shouldEqual` Left "Expected 10 more bytes in input"
    ...
    it "should succeed on valid input" do
      let
        ...
        input = { idx: 0, arr }
        result = runParser parseKV input
      result `shouldEqual` (Right $ Tuple { key: "AB", value: "def" } { idx: 8, arr })

  describe "parseFile" do

    it "should fail on empty input" do
      runParser parseFile { idx: 0, arr: [] } `shouldEqual` Left "failed to get next byte"
    ...
    it "should succeed on valid input" do
      let
        ...
        input = { idx: 0, arr }
        result = runParser parseFile input
        expectedResult =
          { version: 10
